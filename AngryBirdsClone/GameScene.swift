...
//  AngryBirdsClone
//
//  Created by Berke Baç on 21.04.2022.
//
...
class GameScene: SKScene , SKPhysicsContactDelegate {
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        /* fly
        bird.physicsBody?.applyImpulse(CGVector(dx: 10, dy: 200))
                    ...
                    for node in touchNodes {
                        if let sprite = node as? SKSpriteNode{
                            if sprite == bird {
                                ...
                                let dx = -(touchLocation.x - originPosition!.x)
                                let dy = -(touchLocation.y - originPosition!.y)
